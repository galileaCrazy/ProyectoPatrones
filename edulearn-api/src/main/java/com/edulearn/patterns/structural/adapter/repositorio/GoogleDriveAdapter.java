...
/**
 * PATRÓN ADAPTER - Adaptador para Google Drive
 * ============================================
 * Adapta la API específica de Google Drive a nuestra interfaz común.
 *
 * En producción, este adaptador usaría la Google Drive API
 * para crear carpetas, compartir archivos, etc.
 */
@Component
public class GoogleDriveAdapter implements IntegracionExterna {

    private static final Logger logger = LoggerFactory.getLogger(GoogleDriveAdapter.class);
    ...
    public String crearSesion(Map<String, Object> datos) {
        try {
            ...
            String folderId = "folder-" + UUID.randomUUID().toString().substring(0, 16);

            // Extraer datos
            String nombre = (String) datos.getOrDefault("nombre", "Carpeta sin nombre");
            String descripcion = (String) datos.getOrDefault("descripcion", "");
            Boolean esPublica = (Boolean) datos.getOrDefault("esPublica", false);
            String permisos = (String) datos.getOrDefault("permisos", "edit"); // view, comment, edit

            // Crear link directo para abrir Google Drive
            // Este link redirige al usuario a su Google Drive donde puede crear carpetas
            String shareUrl = "https://drive.google.com/drive/my-drive";
            ...
            carpetaInfo.put("proveedor", "GOOGLE_DRIVE");
            ...
            logger.info("✅ Carpeta de Google Drive creada: {} - {}", folderId, nombre);

            return shareUrl;
        } catch (Exception e) {
            logger.error("❌ Error al crear carpeta de Google Drive: {}", e.getMessage());
            throw new RuntimeException("Error al crear carpeta de Google Drive", e);
        }
    }
    ...
    public boolean actualizarSesion(String identificador, Map<String, Object> datos) {
        ...
        logger.info("✅ Carpeta de Google Drive actualizada: {}", identificador);
        return true;
    }
    ...
    public boolean eliminarSesion(String identificador) {
        ...
        if (removed != null) {
            logger.info("✅ Carpeta de Google Drive eliminada: {}", identificador);
            return true;
        }
    ...
    public boolean validarCredenciales(Map<String, String> credenciales) {
        ...
        if (accessToken == null || accessToken.isEmpty()) {
            logger.warn("⚠️ Credenciales inválidas para Google Drive");
            return false;
        }

        logger.info("✅ Credenciales de Google Drive validadas");
        return true;
    }
    ...
    public boolean subirArchivo(String folderId, String nombreArchivo, long tamanoBytes) {
        ...
        logger.info("📄 Archivo subido a Google Drive: {} en carpeta {}", nombreArchivo, folderId);
        return true;
    }
